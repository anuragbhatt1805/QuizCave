export const Question = ({ Question, number, setNext, result, appeared }) => {
  return (
    <>
            ...
            (Question?.type === "multiple") && (
                <>
                    {Question.multipleQuestion.map((option, index) => (
                        <div key={index} className="grid grid-cols-[30%_70%] w-1/3 items-center mx-10">
                            <label htmlFor={index} className="text-lg text-center font-semibold w-full">{`${index+1}) ${option}`}</label>
                            <input
                                type="text"
                ...
                <>
                    <div className="flex flex-col mx-10 my-4">
                        {Question.mcqOptions.map((option, index) => (
                            <div key={index} className="flex flex-row w-1/3 items-center justify-center">
                                <input
                                    type="radio"
                                    name="mcq"
                                    id={index}
                                ...
                                />
                                <label htmlFor={index} className="text-lg text-left font-semibold w-full">{option}</label>
                            </div>
                        ))}
                    </div>
                </>
      ...
      <div>
        <button type="button" className="bg-gray-400 text-black font-bold p-2 rounded-lg m-5 w-32 hover:border-black hover:bg-white border-2" onClick={() => setNext((prev) => prev + 1)}>
            Next
        </button>
        <button type="button" 
